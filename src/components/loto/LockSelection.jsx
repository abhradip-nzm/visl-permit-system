...
export default function LockSelection({ navigate, params }) {
  ...
  const [released, setReleased] = useState(permit.lotoStatus === 'complete' ? false : false);
  ...
  function acknowledgeLockout() {
    ...
    addTimelineEvent(permit.id, `LOTO applied — ${selectedLock.id}`, 'A. Singh (LOTO Responsible)');
    pushToast('Lock reserved system-wide — LOTO acknowledged');
  }

  function releaseLock() {
    updatePermit(permit.id, { lotoStatus: 'released' });
    addTimelineEvent(permit.id, `LOTO released — ${selectedLock?.id || 'lock'}`, 'A. Singh (LOTO Responsible)');
    setReleased(true);
  ...
  return (
    <div className="px-4 py-4">
      <button onClick={() => navigate('tasks')} className="mb-3 flex items-center gap-1.5 text-sm font-semibold text-slate-500">
      ...
      {!isComplete && !signed ? (
        <>
          <Card className="mb-4 p-4">
            <SectionLabel>Select Certified Lock Device</SectionLabel>
