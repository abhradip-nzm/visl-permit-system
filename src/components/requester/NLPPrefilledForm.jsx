...
import { ArrowLeft, Sparkles, Paperclip, FileText, CheckCircle2 } from 'lucide-react';
...
export default function NLPPrefilledForm({ source, navigate, onBack }) {
  ...
  function submit() {
    ...
    pushToast('Permit submitted for approval');
    setTimeout(() => navigate('mypermits'), 900);
  }
  ...
  return (
    <div className="px-4 py-4">
      ...
      <div className="mb-4 flex items-center gap-2 rounded-lg bg-nz-blue-light px-3 py-2 text-xs font-medium text-nz-blue-dark">
        <Sparkles size={14} /> {SOURCE_LABEL[source]}
      </div>
      ...
      <Button variant="orange" size="lg" className="w-full" disabled={!allAcked || !!signed} onClick={submit}>
        {signed ? <><CheckCircle2 size={16} /> Submitted</> : 'Submit for Approval'}
      </Button>
