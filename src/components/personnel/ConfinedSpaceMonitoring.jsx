        ...
        <div className="space-y-1.5">
          {personalEntry.map((p, i) => (
            <div key={i} className="grid grid-cols-3 gap-1.5">
              <input placeholder="Name" value={p.name} onChange={(e) => setPersonalEntry((prev) => prev.map((r, idx) => idx === i ? { ...r, name: e.target.value } : r))} className="rounded-lg border border-nz-border px-2 py-1.5 text-xs focus-ring" />
