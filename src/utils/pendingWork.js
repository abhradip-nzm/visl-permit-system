  ...
  switch (role) {
    ...
    case 'hod':
      return permits.filter(
        (p) => p.status === 'pending-clearance' && needsClearance(p.types || [p.type]) && (!department || p.deptClearances?.[department]?.status === 'pending')
      ).length;
    case 'approver':
    ...
    case 'worker':
      return permits.filter((p) => p.status === 'live' && p.workers?.some((w) => w.name === currentUser.name && !w.applied)).length;
    case 'rescuer':
